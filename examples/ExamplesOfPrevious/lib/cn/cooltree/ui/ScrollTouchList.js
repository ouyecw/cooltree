...
Global.inherit(ScrollTouchList,Sprite);
ScrollTouchList.SELECT="stl_select_item";

Object.defineProperty(ScrollTouchList.prototype,"data",{
	get: function (){
		return this._datas;
	},
	/**
...
Object.defineProperty(ScrollTouchList.prototype,"listWidth",{
	get: function (){
    ...
    set: function (value) {
		...
		if(this._portrait){
			...
			for(var i in this._list) this._list[i].itemWidth=this._list_width;
...
Object.defineProperty(ScrollTouchList.prototype,"listHeight",{
	get: function (){
    ...
    set: function (value) {
		...
		if(!this._portrait){
			...
			for(var i in this._list) this._list[i].itemHeight=this._list_height;
			this.mask=new Rectangle(0,0,this._list_width,this._list_height);
			return;
		}
...
Object.defineProperty(ScrollTouchList.prototype,"current",{
	get: function (){
 ...
 */
ScrollTouchList.prototype.setup=function(width,height,item_size,item_class,portrait,color,index)
{
 ...
 * 清除列表
 */
ScrollTouchList.prototype.clear=function()
{
	if(this._datas==null || this._list==null) return;
	
	var i,l=this._list.length,item;
...
ScrollTouchList.prototype.init=function(index)
{
...
//  var line=Factory.c("bs",["#000000",this._list_width+20,2]);
...
ScrollTouchList.prototype.onMouseDown=function(e)
{
...
ScrollTouchList.prototype.onMouseMove=function(e)
{
	var point=new Point(e.mouseX,e.mouseY);

	if(!this._has_drag && Point.distance(this._start_point,point)>5){
		var pos=this.localToGlobal(this._portrait ? 0 : this._middle,this._portrait ? this._middle : 0);
...
ScrollTouchList.prototype.onMouseUp=function(e)
{
	...
	var delay=new Date().getTime()-this._time;
    ...
    this._start_point=this._temp_point=this._time=null;
	var bool=this._portrait ? (this._container.y>this._middle || this._container.y<this._middle-this._length+this._item_size) : (this._container.x>this._middle || this._container.x<this._middle-this._length+this._item_size);
	if(!bool) return;
	
	var posX=this._portrait ? 0 : (this._container.x>this._middle ? this._middle : (this._container.x<this._middle-this._length+this._item_size ? this._middle-this._length+this._item_size : this._container.x));
	var posY=this._portrait ? (this._container.y>this._middle ? this._middle : (this._container.y<this._middle-this._length+this._item_size ? this._middle-this._length+this._item_size : this._container.y)) : 0;
	TweenLite.to(this._container,0.16,{ease:"easeoutquad",x:posX,y:posY,onComplete:this._complete});
}

ScrollTouchList.prototype._move_list_by_speed=function(speed)
{
	...
	var offset=0.3;
	var num=MathUtil.format(speed*(this._item_size+this.space)*this.offset);
	var time=Math.max(0.1,MathUtil.format((num*this.offset*offset)/this._item_size));
 ...
 */
ScrollTouchList.prototype.moveList=function(num,time)
{
	if(num==null || num==0) return;
	var label=(this._portrait ? "y" : "x");
	var pos=Math.ceil((this._container[label]+(this._is_next ? -1 : 1)*num)/(this._item_size+this.space));
...
ScrollTouchList.prototype.onTweenComplete=function()
{
	this._update_items_state();
	this._move_current_item();
...
ScrollTouchList.prototype.onMouseWheel=function(e)
{
...
ScrollTouchList.prototype.onMouseTap=function(e)
{
	var item=e.target;
	if(item==null ) return;
 ...
 */
ScrollTouchList.prototype.goto=function(index,skip)
{
	...
	var temp=((index+1)*(this._item_size+this.space))-(index>0 ? this.space : 0);
	temp=temp+this._container[label]-this._middle-this._item_size;
	this._is_next=(temp>0);
	
	var offset=0.1;
	var num=Math.abs(temp);
	var time=skip ? 0 : Math.max(0.1,MathUtil.format((num*this.offset*offset)/this._item_size));
	this.moveList(num,time);
}
 ...
 * 根据列表位置 刷新显示内容
 */
ScrollTouchList.prototype.reset_list=function()
{
	if(this._list==null || this._list.length<1) return;
	var i,j,l=this._list.length,item;
	var label=(this._portrait ? "y" : "x");
	var offset=this._container[label];
	var index=Math.abs(Math.ceil((this._middle-offset+this._item_size*0.5)/(this._item_size+this.space)));
	
	index=index>0 ? index-1 : 0;
	var half=Math.ceil(this._max_num*0.5)-(this._max_num%2==0 ? 0 : 1);
...
ScrollTouchList.prototype._create_list=function()
{
	if(this._datas==null || this._datas.length<1) return;
	var i,data,item,len=Math.min(this._datas.length,this._max_num);
...
ScrollTouchList.prototype._update_list=function()
{
	this._update_items_state();
	if(!this._is_full) return;
	
	var end=(this._is_next ? this._list[this._list.length-1] : this._list[0]);
	var index=end.index;
	
	if((this._is_next && index>=(this._datas.length-1)) || (!this._is_next && index<=0)) return;
	
	var item=(this._is_next ? this._list[0] : this._list[this._list.length-1]);
	var point={x:(item.x+this._container.x),y:(item.y+this._container.y)}
	var label=(this._portrait ? "y" : "x");
...
ScrollTouchList.prototype._update_items_state=function(index)
{
	if(this._list==null || this._list.length<1) return;
	
	var target=(index==null) ? this._middle : (this._list[index][label]+this._container[label]);
	var i,j=0,l=this._list.length,item,min,dy;
	var label=(this._portrait ? "y" : "x");
...
ScrollTouchList.prototype._move_current_item=function()
{
	var label=(this._portrait ? "y" : "x");
	var offest=(this._temp_item[label]+this._container[label]-this._middle);
	this._container[label]-=offest;
	this.dispatchEvent(new Event(ScrollTouchList.SELECT,this._datas[this._current]));
}

ScrollTouchList.prototype.dispose=function()
{
	this.clear();
	ScrollTouchList.superClass.dispose.call(this);
