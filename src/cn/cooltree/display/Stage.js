...
class Stage extends DisplayObjectContainer
{
	...
	size(w,h)
	{
		...
		if(Global.autoShapeSize){
			this.dispatchEvent(Factory.c("ev",[StageEvent.UPDATE]));
		}
	}
	...
	enable(bool)
	{
		...
		if(bool){
			Global.root.addEventListener('message', this.__message_handler, false);
			document.addEventListener("keydown", this.__key_handler, false);
			document.addEventListener("keyup", this.__key_handler, false);
		}else{
			Global.root.removeEventListener('message', this.__message_handler, false);
	...
	{	
		...
		let i,t,type,mx,my,l=e.touches.length;
	    ...
	    let bt=this._checkMouseClick(a);
