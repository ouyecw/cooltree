...
class Transformer extends DisplayObjectContainer
{
	constructor()
	{
		super();
	...
	_mouse_wheel_handle(e)
	{
		...
		this._btn.moveTo(this.globalToLocal(this._canvas.localToGlobal(this._point.x,this._point.y)));
		this.dispatchEvent(Factory.c("ev",[Transformer.SCALE]));
	...
	_key_up(e)
	{
		if(this._has_key_down) {
			...
			this.dispatchEvent(Factory.c("ev",[Transformer.MOVE]));
		}
	...
	_key_down(e)
	{
		...
		switch(e.params){
			...
			case 46:
			this.dispatchEvent(Factory.c("ev",[Transformer.DELETE]));
			return;
		}
	...
	_drag_mouse_move(e)
	{
		...
		DisplayUtil.copyTransform(this._canvas,this._target,this.relative_data);
		this._btn.moveTo(this.globalToLocal(this._canvas.localToGlobal(this._point.x,this._point.y)));
		this.dispatchEvent(Factory.c("ev",[Transformer.SCALING]));
	...
	_drag_mouse_up(e)
	{
	    ...
	    else if(this._state==1 && Math.sqrt((this._cache_point.x-this._canvas.x)*(this._cache_point.x-this._canvas.x)+(this._cache_point.y-this._canvas.y)*(this._cache_point.y-this._canvas.y))<3) this.dispatchEvent(Factory.c("ev",[Transformer.CLICK]));
