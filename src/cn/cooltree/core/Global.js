...
class Global
{
	...
	*/
	...
	static get local()
	{
		...
		let local=Global.root.localStorage;
		local=(local==undefined || local==null) ? Global.root.sessionStorage : local;
		
		if(local==undefined || local==null){
			if(Global.__local_cache==undefined)  Global.__local_cache=Object.create(null);
			local = Global.__local_cache;
		}
		
		Global.__local=new StorageObject(local);
...
Global._gc_list=["Event","StageEvent","Shape","DisplayObjectContainer","Graphics","DisplayObject","DOMDisplay","MovieClip","DOMMovie","BoxShape"];//,"Matrix","Source","Effect"
