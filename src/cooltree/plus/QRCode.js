...
export default class QRCode
{
	 ...
	 * @param {String} text
	 * @param {Object} option 
	 * 		   {width,height,typeNumber,colorLight,colorDark,correctLevel}
	 *         correctLevel L:1,M:0,Q:3,H:2
	 */
	constructor(text=null,defautOption=null) 
	{
	    this.option = {
			width : 256,
			height : 256,
		...
		this.model=null;
		if(defautOption) Object.assign(this.option,defautOption);
		this.makeCode();
	}
	
	makeCode(text=null)
	{
		if(text && this.option && !this.option.text) this.option.text=text;
		if(!this.option || !this.option.text) return this;
		this.model = new QRCodeModel(this._getTypeNumber(this.option.text, this.option.correctLevel), this.option.correctLevel);
		this.model.addData(this.option.text);
		this.model.make();
		return this;
	}
	...
	draw(context)
	{
		...
		if(context instanceof ContextVO){
			context.strict=true;
		}
		
		const nCount = this.model.getModuleCount();
		const nWidth = this.option.width / nCount;
		const nHeight = this.option.height / nCount;
		const nRoundedWidth = Math.round(nWidth);
		const nRoundedHeight = Math.round(nHeight);
		
		for (var row = 0; row < nCount; row++) {
			for (var col = 0; col < nCount; col++) {
				var bIsDark = this.model.isDark(row, col);
				var nLeft = col * nWidth;
				var nTop = row * nHeight;
				context.strokeStyle = bIsDark ? this.option.colorDark : this.option.colorLight;
				context.lineWidth = 1;
				context.fillStyle = bIsDark ? this.option.colorDark : this.option.colorLight;
...
class QRMath
{
	...
	{
		if(QRMath._EXP_TABLE) return QRMath._EXP_TABLE;
		QRMath._EXP_TABLE=new Array(256);
		...
		for (var i = 8; i < 256; i++) {
		    QRMath._EXP_TABLE[i] = QRMath._EXP_TABLE[i - 4] ^ QRMath._EXP_TABLE[i - 5] ^ QRMath._EXP_TABLE[i - 6] ^ QRMath._EXP_TABLE[i - 8];
		}
		
		return QRMath._EXP_TABLE;
	}
	
	static get LOG_TABLE()
	{
		if(QRMath._LOG_TABLE) return QRMath._LOG_TABLE;
		QRMath._LOG_TABLE=new Array(256);
		
		for (var i = 0; i < 255; i++) {
		    QRMath._LOG_TABLE[QRMath.EXP_TABLE[i]] = i;
		}
		
		return QRMath._LOG_TABLE;
	}
}

QRMath._EXP_TABLE=QRMath._LOG_TABLE=null;
