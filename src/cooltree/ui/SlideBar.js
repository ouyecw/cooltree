...
export default class SlideBar extends MoveBar
{
	...
	_activate_bar(bool)
	{
		...
		if(bool){
			if(this._target) {
				this._target.on(Event.RESIZE,Global.delegate(this._reset_handler,this),this.name);
				this._target.on(DisplayBase.RESIZE,Global.delegate(this.resetRange,this),this.name);
				this._target.on(StageEvent.DRAG_MOVE,Global.delegate(this.dragHandler,this),this.name);
				this._target.on(DisplayBase.RESET_INSTANCE,Global.delegate(this.resetRange,this),this.name);
				if(this.useWheel) this._target.on(StageEvent.MOUSE_WHEEL,Global.delegate(this.wheelHandler,this),this.name);
			}
			
			if(this._up_btn) this._up_btn.on(StageEvent.MOUSE_CLICK,Global.delegate(this._clickUpBtn,this));
			if(this._down_btn) this._down_btn.on(StageEvent.MOUSE_CLICK,Global.delegate(this._clickDownBtn,this));
			return;
		}
		
		if(this._target) {
			this._target.off(Event.RESIZE,null,this.name);
			this._target.off(DisplayBase.RESIZE,null,this.name);
			this._target.off(StageEvent.DRAG_MOVE,null,this.name);
			this._target.off(StageEvent.MOUSE_WHEEL,null,this.name);
			this._target.off(DisplayBase.RESET_INSTANCE,null,this.name);
		}
		
		if(this._up_btn) this._up_btn.off(StageEvent.MOUSE_CLICK);
		if(this._down_btn) this._down_btn.off(StageEvent.MOUSE_CLICK);
	}
