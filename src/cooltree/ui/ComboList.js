...
{	
	...
	set current(value) 
	{
    	...
    	this._current=value;
    	if(bool) this.emit(new Event(ComboList.SELECT,this._value)); 
    }
	...
	initialize()
	{
		...
		this._list.on(StageEvent.MOUSE_OVER,Global.delegate(this._stopCountTime,this),this.name);
		this._list.on(StageEvent.MOUSE_OUT,Global.delegate(this._startCountTime,this),this.name);
		
		this._btn.on(StageEvent.MOUSE_CLICK,Global.delegate(this._onClickHandler,this),this.name);
	    this._btn.on(StageEvent.MOUSE_OVER,Global.delegate(this._stopCountTime,this),this.name);
		this._btn.on(StageEvent.MOUSE_OUT,Global.delegate(this._startCountTime,this),this.name);
	...
	addItem(data,noRefresh)
	{
		...
		btn.on(StageEvent.MOUSE_TAP,Global.delegate(this._onTapHandler,this),this.name);
	...
	_inCompleteHandle()
	{
		this._block=false;
		this._sprite.visible=false;
		this.emit(new Event(ComboList.SPREAD,false));
	}
	
	_outCompleteHandle()
	{
		this._block=false;
		this.emit(new Event(ComboList.SPREAD,true));
	}
	...
	dispose()
	{
		...
		if(this._btn){
			this._btn.off(StageEvent.MOUSE_CLICK,null,this.name);
	   		this._btn.off(StageEvent.MOUSE_OVER,null,this.name);
		    this._btn.off(StageEvent.MOUSE_OUT,null,this.name);
		}
