...
{
	static className="TextField";
	static specialCode=[183,215,8212,8216,8217,8220,8221,8230,12289,12290,12298,12299,12302,12303,12304,12305,65281,65288,65289,65292,65306,65307,65311];
	...
	get dimension()
	{
		if(StringUtil.isEmpty(this._text)) 
			return {width:0,height:0};
		
		this._update();
		return {width:Math.min(this.width,this.lineWidth),height:this.height};
	}
	...
	set text(value) 
	{
        if(this._text==value || typeof value!="string") return;
	...
	set stroke(value) 
	{
		if(this._stroke==value || value<0) return;
		this._stroke=value;
	}
	...
	holdHeight(value,suffix="..")
	{
		this._update();
		if(value<=0 || this.getHeight()<=value) return;

		const num=suffix.length+1;
		let count=0;

		while(this.getHeight()>value && count<80)
		{
			this.text=this.text.slice(0,this.text.length-num)+suffix;
			this._update();
			count++;
		}
	}
	...
	getTextWidth(str=null)
	{
		...
		const stage=this.stage || Global.stage;

		if(!stage || !stage.context || !this.context) return this._size;
		if(stage.context.font !=this.context.font) stage.context.font=this.context.font;

		let width;
		try{
			width=stage.context.measureText(str).width
		}catch(err){
			width=TextField.measureText(str,this._size);
		}
	...
	_vertical_text()
	{
		...
		let isNormal,word,i,j,line,l=texts.length, width,oy=this._size * this._lineHeight*0.5,dy =oy,my=(this.autoSize && this._lineWidth ? this._lineWidth : this.height), sy=this._size*0.5, wordHeight = this._size;
		...
		for (i=0;i<l;i++){
			...
			for(j = 0, wlen = line.length; j < wlen; j++)
			{
				...
				if(sy+dy>=my-this._leading){
					...
					dy=oy;
					mx-=this._size*this._lineHeight;
					if(!this.autoSize && mx<=this._leading) break;
				}
	...
	{
		const texts=this._text.split(/\r\n|\r|\n|<br(?:[ \/])*>/);
		let i,cx,line,l=texts.length,lineHeight = this._size * this._lineHeight, dy = this._size * this._lineHeight*0.5;
	...
	_draw(str,posY=0,posX=0,vertical=false)
	{
		...
		if((this._fill_type=="stroke" || this._fill_type=="both") && (this.context.lineWidth != this._stroke))
			this.context.lineWidth=this._stroke;
	...
	{
		super.dispose();
		delete this._stroke,this.wordBreak,this.scrollHeight,this._lineHeight,this.autoSize,this._underline,this._writingMode,this._text,this._font,this._color,this._color2,this._size,this._textAlign,this._textBaseline,this._fill_type,this._has_update,this._leading,this._italic,this._bold,this._lineWidth,this.mouseEnabled;
	}
	...
	static measureText (text, fontSize=10) 
	{
		text = String(text);
		var text = text.split('');
		var width = 0;
