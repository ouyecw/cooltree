...
{
	static className="DisplayBase";
	static RESIZE="display_base_resize";
	static RESET_INSTANCE = "display_reset_instance";
	...
	set origin(value) 
	{
		...
		this.register_point.x=MathUtil.format(value.x);
		this.register_point.y=MathUtil.format(value.y);
	...
	getMatrix (target,isDraw)
	{
		...
		if(!this._temp_matrix) 
			this._temp_matrix=ObjectPool.create(Matrix);
		else 
		...
		const originX=this.origin ? this.origin.x : 0;
		const originY=this.origin ? this.origin.y : 0;

		this._temp_matrix.concatTransform(this.x, this.y, this.scaleX, this.scaleY, this.rotation, originX ,originY ,this.skewX,this.skewY);
	...
	reset ()
	{
		...
		this[_scaleY]=this[_scaleX]=this._alpha=this._parent_alpha=1;
	    this._refresh=this._resize=this.mouseEnabled=this.usePolyCollision=this.breakTouch=false;
	...
	dispose ()
	{
		this.reset();
		super.dispose();
		delete this._refresh,this._bounds,this._minX,this._minY,this._temp_matrix,this.breakTouch,this._resize,this[_skewX],this[_skewY],this._stage,this._updateMatrix,this.polyArea,this.usePolyCollision,this._matrix,this.register_point,this[_height],this[_width],this[_y],this[_x],this[_rotation],this[_scaleY],this[_scaleX],this._alpha,this.mouseEnabled,this._visible,this.name,this._parent,this._parent_alpha;
	}
