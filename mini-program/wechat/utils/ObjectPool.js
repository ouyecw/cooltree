...
export default class ObjectPool
{
	...
	static create(value,args)
	{
		...
		if(item) item.__class_name=type;
		return item;
	}
	...
	static remove(obj)
	{
		...
		type=(!type || type.length<3) ? obj.__class_name : type;
		...
		try{
			...
			// }
			// catch(e) {
			// 	console.log("[WARN] ObjectPool.remove dispose class by",type,"detail:"+e.toString());
			// }
			return false;
		}
		
		if(ObjectPool.max>0 && (ObjectPool._dic[type]).length>=ObjectPool.max){
			// console.warn("[ObjectPool] "+type+" is overflowing.");
			try{
				obj.dispose();
			}
			catch(e) {};
