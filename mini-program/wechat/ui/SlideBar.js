...
export default class SlideBar extends MoveBar
{
	...
	_activate_bar(bool)
	{
		...
		if(bool){
			if(this._target) {
				this._target.on(Event.RESIZE,this._reset_handler,this,this.name);
				this._target.on(DisplayBase.RESIZE,this.resetRange,this,this.name);
				this._target.on(StageEvent.DRAG_MOVE,this.dragHandler,this,this.name);
				this._target.on(DisplayBase.RESET_INSTANCE,this.resetRange,this,this.name);
				if(this.useWheel) this._target.on(StageEvent.MOUSE_WHEEL,this.wheelHandler,this,this.name);
			}
			
			if(this._up_btn) this._up_btn.on(StageEvent.MOUSE_CLICK,this._clickUpBtn,this);
			if(this._down_btn) this._down_btn.on(StageEvent.MOUSE_CLICK,this._clickDownBtn,this);
			return;
		}
