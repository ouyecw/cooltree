...
export default class TweenLite
{
	...
	static to  (target, duration, vars) 
	{
		...
		if(TweenLite._tweens.length==0 && !TweenLite._locked){
			TweenLite.resume(); 
		}
		
		const fps=Math.floor(Global.fps/Global.multiple);
		const data={target : target, duration : duration, vars : vars};
		
		data.max=Math.ceil(duration*fps);
		data.min=0;
		
		if(data.vars.hasOwnProperty("delay") && data.vars["delay"]){
			data.delay=Math.ceil(Number(data.vars["delay"])*fps);
	...
	{
		if(Global.stage){
			Global.stage.off(StageEvent.ENTER_FRAME,null,"tween-lite");
		}
	}
	
	static resume  () 
	{
		if(Global.stage && !Global.stage.has(StageEvent.ENTER_FRAME,"tween-lite")){
			Global.stage.on(StageEvent.ENTER_FRAME,TweenLite._enterFrame,null,"tween-lite");
		}
	}
	...
	static remove  (target) 
	{
		...
		let i,tween;
		let len=TweenLite._tweens.length;
	...
	//			console.log("[ERROR] TweenLite._setAttribute:onStart "+twname+"{"+err+"}");
	//		}
	...
	//			catch(err){
	//				twname=twname || target.name+"("+ClassUtil.getQualifiedClassName(target)+")";
	//				console.log("[ERROR] TweenLite._setAttribute:onComplete "+twname+"{"+err+"}");
	//			}
			}
			return;
		}
	...
	//		catch(err){
	//			twname=twname || target.name+"("+ClassUtil.getQualifiedClassName(target)+")";
	//			console.log("[ERROR] TweenLite._setAttribute:onUpdate "+twname+"{"+err+"}");
	//		}
		}
	}
