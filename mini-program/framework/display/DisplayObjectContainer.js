...
{
	static className="DisplayObjectContainer";
	...
	getChildByPath(path)
	{
		...
		while(array.length>0 && current && (current instanceof DisplayObjectContainer)){
			name=array.shift();
	...
	async cacheAsImage(width=0,height=0)
	{
		...
		const base64=CanvasUtil.containerToImage(this);
		const image=await Loader.loadImage(base64);
		this.removeAllChildren(true);
		
		const display_obj=ObjectPool.create(DisplayObject);
		const source=ObjectPool.create(Source);
		source.image=image;
		source.isClone=true;
		source.width=width;
		source.height=height;
		display_obj.setInstance(source);
	...
	{
		super.dispose();
		delete this._stage,this.autoSize,this._children,this.mouseChildren;
	}
