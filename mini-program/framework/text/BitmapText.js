...
{	
	...
	//	let array=this._fonts.info.padding.split(",");
		...
		let char,pos,len = this._text.length, charCode = -1,h=0,w=0;
		let old=ObjectPool.create(Point);
		let miss="";
		
		this._instance.context.reset();
		...
		for (i = 0; i < len; i++) {
		    ...
		    if(countX>=(this._lineWidth<=0 ? this.stage.stageWidth : this._lineWidth)-char.width){
		    	if(i+1 < len && !this.wordBreak){
		...
		if(!StringUtil.isEmpty(miss)) 
		      console.log("[WARN] _update miss fonts ["+miss+"]");
	...
	dispose()
	{
		...
		delete this.wordBreak,this.autoSize,this._lineHeight,this._size,this._ct,this._color,this._is_init,this._has_update,this._lineWidth,this._leading,this._text,this._font,this._textAlign,this._instance;
	}
}
