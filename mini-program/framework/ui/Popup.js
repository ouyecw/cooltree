...
export default class Popup
{
	constructor(config=null)
	{
		...
		this.itemWidth=(Global.stage.stageWidth<this.config.maxWidth ? Global.stage.stageWidth : this.config.maxWidth)*this.config.percent;
	...
	push(str)
	{
		...
		if(this.headY<1 && Global.system){
			this.headY=Global.system.screenTop+Global.system.statusBarHeight;
		}
		
		if(!this.popupContainer.parent){
			Global.stage.addChild(this.popupContainer);
			this.resize(Global.stage.stageWidth);
		}
		...
		TweenLite.to(this.popupContainer,0.4,{y:this.headY+this.posY});
	...
	_create(content)
	{
		...
		item.addChild(tf);
		tf.moveTo((this.itemWidth-(tf.width || tf.lineWidth))*0.5,this.config.space);
		return item;
	}
