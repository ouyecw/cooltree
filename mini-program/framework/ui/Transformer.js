...
export default class Transformer extends DisplayObjectContainer
{
	...
	_update_target_size()
	{
		...
		this._canvas.instance.moveTo(-this.lineWidth*0.5,-this.lineWidth*0.5);
	...
	_init_transformer()
	{
		...
		this._btn.on(StageEvent.MOUSE_DOWN,this._trans_mouse_down,this,this.name);
	    this._canvas.on(StageEvent.MOUSE_DOWN,this._drag_mouse_down,this,this.name);
	
		this._canvas.matrix=this._target.matrix;
		this._btn.moveTo(this.globalToLocal(this._canvas.localToGlobal(this._point.x,this._point.y)));
		
		if(Global.isPC){
			if(this.wheel_control) this._canvas.on(StageEvent.MOUSE_WHEEL, this._mouse_wheel_handle,this,this.name);
		}
	}
	...
	_drag_mouse_down(e)
	{
		...
		if(this.support_finger && this._double_finger){
			...
			(this.stage ? this.stage : Global.stage).startDrag(this._canvas);
		}
		
		(this.stage ? this.stage : Global.stage).on(StageEvent.MOUSE_UP, this._drag_mouse_up,this,this.name);
		(this.stage ? this.stage : Global.stage).on(StageEvent.MOUSE_MOVE, this._drag_mouse_move,this,this.name);
	}
	...
	_trans_mouse_down(e)
	{
		...
		(this.stage ? this.stage : Global.stage).on(StageEvent.MOUSE_UP, this._drag_mouse_up,this,this.name);
		(this.stage ? this.stage : Global.stage).on(StageEvent.MOUSE_MOVE, this._trans_mouse_move,this,this.name);
	}
	...
	_drag_mouse_up(e)
	{
		(this.stage ? this.stage : Global.stage).stopDrag();
		(this.stage ? this.stage : Global.stage).off(StageEvent.MOUSE_UP, null,this.name);
		(this.stage ? this.stage : Global.stage).off(StageEvent.MOUSE_MOVE, null,this.name);
	...
	dispose()
	{
		...
		delete this.support_finger,this._min_scale,this.min_size,this.wheel_control,this._has_key_down,this.is_free,this.pos_percent,this.rotate,this.round,this.lineWidth,this._target_center,this._move_point,this._target_angle,this._target_rotation,this._target_scaleX,this._target_scaleY,this._target_distance,this._target_offsetX,this._target_offsetY;
	}
}
