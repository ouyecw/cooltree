...
{
	static DRAG_MOVE="drag_move_container";
	static DRAG_OUT="drag_out_container";
	static className="UIContainer";
	...
	{	
		...
		this._register_instance.on(DisplayBase.RESIZE,this._reset_hold_control,this,this.name);
	...
	_reset_hold_control(e)
	{
		...
		this._register_instance.mouseEnabled=bool;
		this.scroll=bool;
		
		if(bool) {
			if(!this._register_instance.has(StageEvent.MOUSE_DOWN))
				this._register_instance.on(StageEvent.MOUSE_DOWN,this._mouse_drag_handler,this,this.name);
	    }
		else {
	...
	_mouse_drag_handler(e)
	{
		...
		this._register_instance.on(StageEvent.DRAG_MOVE,this._drag_move_handler,this,this.name);
		
		(this.stage ? this.stage : Global.stage).startDrag(this._register_instance,rect,false,free);
		(this.stage ? this.stage : Global.stage).on(StageEvent.MOUSE_UP,this._mouse_up_handler,this,this.name);
	}
	...
	_mouse_up_handler(e)
	{
		if(e){
			...
			if(this._overflow){
				...
				if(posX!=this._register_instance.x || posY!=this._register_instance.y) {
					bool=false;
					const rx=this._register_instance.x;
					const ry=this._register_instance.y;
					TweenLite.to(this._register_instance,this.back_time,{ease:this.back_ease,x:posX,y:posY,
						...
						onComplete:()=>{
							const obj={};
							if(posX!=rx) obj.x=(posX>rx);
							if(posY!=ry) obj.y=(posY>ry);
							this.emit(new Event(UIContainer.DRAG_OUT,obj));
						}
					});
				}
			}
		...
		(this.stage ? this.stage : Global.stage).mouseTarget=null;
