...
export default class StringUtil
{
	...
	static parseParams(str,is_style=false,type=true)
	{
		if(StringUtil.isEmpty(str)) return {};
		let i,l,name,datas,params=str.split(!is_style ? "&" : ";"),data={},decode=(type ? decodeURIComponent : unescape);
		
		for(i=0,l=params.length;i<l;i++)
		{
			datas=params[i];
			if(StringUtil.isEmpty(datas) || (!is_style && datas.indexOf("=")<0) ||  (is_style && datas.indexOf(":")<0)) continue;
			datas=datas.split(!is_style ? "=" : ":");
			name=StringUtil.trim(datas[0]);
			if(is_style && name && name.indexOf("-")>0) name=StringUtil.decodeName(name);
	...
	static buildParams(obj,is_style=false,type=1)
	{
		if(obj==null) return "";
	    ...
	    if (node.attributes.length > 0) {
	        for (let j = 0; j < node.attributes.length; j++) {
	            ...
	            if(nodeName.toLowerCase()=="style"){
	            	obj[nodeName] = StringUtil.parseParams(obj[nodeName],true);
	            }
	        }
	    }
