...
export default class ArrayUtil
{
	 ...
	 * @param {String} property 
	 */
	static maxNumberByProperty(array,property)
	{
		if(!array || !array.length || !property) return 0;

		let max=0;
		for(let item of array){
			if(item && item.hasOwnProperty(property))
				max=Math.max(max,Number(item[property]));
		}
	...
	static sort(array,property=null,order=false,num=null)
	{
		...
		return MathUtil.sort(array,function(a,b){
	        let value1 = StringUtil.isEmpty(property) ? a : ObjectUtil.getAttribute(a,property);
	        let value2 = StringUtil.isEmpty(property) ? b : ObjectUtil.getAttribute(b,property);
